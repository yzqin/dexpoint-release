...
from hand_teleop.env.rl_env.allegro_env import AllegroRLEnv
...
class AllegroRelocateRLEnv(RelocateEnv, AllegroRLEnv):
    def __init__(self, use_gui=False, frame_skip=10, is_robot_free=False, robot_frame="robot",
                 object_category="YCB", object_name="tomato_soup_can", object_scale=1.0, rotation_reward_weight=0,
                 randomness_scale=1, friction=1, **renderer_kwargs):
        super().__init__(use_gui=use_gui, frame_skip=frame_skip, object_category=object_category,
                         object_name=object_name, object_scale=object_scale, randomness_scale=randomness_scale,
                         friction=friction, **renderer_kwargs, )

        # Choose robot name based on free or not free hand
        if is_robot_free:
            robot_name = "allegro_hand_free"
        else:
            robot_name = "allegro_hand_xarm6_wrist_mounted_face_front"
        self.setup_allegro(robot_name, robot_frame)

        # Object and target information cache
        self.object_pose = self.manipulated_object.get_pose()
        self.palm_pose = self.palm_link.get_pose()
        self.palm_pos_in_base = np.zeros(3)
        self.object_in_tip = np.zeros([len(self.finger_tip_links), 3])
        self.target_in_object = np.zeros([3])
        self.target_in_object_angle = np.zeros([1])
        self.object_lift = 0

        # Contact buffer
        self.robot_object_contact = np.zeros(len(self.finger_tip_links) + 1)  # four tip, palm
    ...
    def get_oracle_state(self):
        ...
        return np.concatenate([
            self.robot_qpos_vec, self.palm_pos_in_base,
    ...
    def get_reward(self, action):
        ...
        reward += np.sum(self.robot_object_contact[:4] * self.finger_reward_scale) * 10
    ...
    def reset(self, *, seed: Optional[int] = None, return_info: bool = False, options: Optional[dict] = None):
        ...
        # Reset
        self.reset_allegro(seed=seed, robot_init_pose=init_pose)
        return self.get_observation()
...
def main_env2():
    env = AllegroRelocateRLEnv(use_gui=True, object_name="mustard_bottle", frame_skip=10, use_visual_obs=False)
    base_env = env
    robot_dof = env.robot.dof
    env.seed(0)
    env.reset()
